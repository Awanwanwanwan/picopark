...
class Key extends Entity {
    ...
    update() {
        // dynamic hover target based on player count: lower with more players
        const playerCount = Math.max(1, this.game.players.length||1)
        const hoverBase = 45
        // More aggressive height reduction for fewer players
        const hoverHeight = Math.max(15, hoverBase - (playerCount*6))
        ...
        if (!this.targetedDoor) {
            if (this.followingPlayer) {
                var newPos = v(this.followingPlayer.body.position.x,this.followingPlayer.body.position.y-((hoverHeight)*this.followingPlayer.scale))
                ...
                this.game.players.forEach((e)=>{
                    if (getDst(e.body.position, this.pos)<45) {
                        ...
                        // 播放收集钥匙音效
                        if (window.soundManager) window.soundManager.play('key')
                    }
                })
            ...
            var newPos = v(this.targetedDoor.trigger.rect.position.x,this.targetedDoor.trigger.rect.position.y-(hoverHeight))
                var rawDst = getDst(newPos, this.pos),
            ...
            if (rawDst<10) {
                ...
                this.unload = true
                // 播放开门音效
                if (window.soundManager) window.soundManager.play('door')
            }
        }
    }
    render(ctx) {
