class Game {
    ...
    async initRender() {
        ...
        await this.renderer.showTitle(`-- ${this.runTemp?"临时关卡":"one"} --`, 500)
    ...
    // When player count changes, refresh level to apply dynamic gap/key
    refreshForPlayerCountChange() {
        if (!this.levelHandler.currentLevel||!this.levelHandler.currentLevel.name) return
        const name = this.levelHandler.currentLevel.name
        this.levelHandler.setLevel(name)
    }
