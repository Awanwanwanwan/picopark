    ...
    loadLevel(dat, name) {
        ...
        var makeParts = (dir)=>{
            ...
            for (let i = 0; i < horizontalBodies.length; i++) {
                ...
                // dynamic gap based on player count: smaller gaps for fewer players
                const playerCount = Math.max(1, this.game.players.length||1)
                // More aggressive gap reduction for fewer players, especially for first level
                const isFirstLevel = (this.currentLevel.name === "one")
                const baseGapSize = isFirstLevel ? 0.3 : 0.4  // Smaller base for first level
                const gapSize = Math.max(baseGapSize, 0.8 - (playerCount*0.15))
                let wall = Matter.Bodies.rectangle(
                    ((bod.pos.x+((1-(levelThickness*gapSize))*0.5)-((levelThickness*gapSize)*Math.min(dir,0)))*cellsize.x),
                    ((bod.pos.y-(bod.length*0.5)-0.5)*cellsize.y), 
                    cellsize.x*(levelThickness*gapSize), 
            ...
            for (let i = 0; i < verticalBodies.length; i++) {
                ...
                let wall = Matter.Bodies.rectangle(
                    ...
                    ((bod.pos.y+1-((1-(levelThickness*gapSize))*0.5)+((levelThickness*gapSize)*Math.min(-dir,0)))*cellsize.y),
